...
void toggleAttachBelow() { attachbelow = !attachbelow; }
...
void deck(Monitor *m) {
  ...
  for (i = 0, my = m->gappx, c = nexttiled(m->clients); c;
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    if (occ & 1 << i)
      drw_rect(drw, x + boxw, 0, w - (2 * boxw + 1), boxw / 4 + 1, 1,
               urg & 1 << i);
...
void tile(Monitor *m) {
  ...
  /* remove gaps if there is only one client
  if(n == 1){
          c = nexttiled(m->clients);
          resize(c, m->wx, m->wy, m->ww - 2 * c->bw, m->wh - 2 * c->bw, 0);
          return;
...
void bstack(Monitor *m) {
  ...
  for (i = 0, mx = tx = m->gappx, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      w = (m->ww - mx) / (MIN(n, m->nmaster) - i) - m->gappx;
      resize(c, m->wx + mx, m->wy + m->gappx, w - 2 * c->bw,
             mh - 2 * c->bw - m->gappx * (5 - ns) / 2, 0);
      if (mx + WIDTH(c) + m->gappx < m->mw)
        mx += WIDTH(c) + m->gappx;
    } else {
      w = (m->ww - tx) / (n - i) - m->gappx;
      if (m->nmaster == 0)
        resize(c, m->wx + tx, m->wy + mh, w - (2 * c->bw),
               m->wh - mh - 2 * c->bw - m->gappx, False);
      else
...
void bstackhoriz(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (i < m->nmaster) {
      resize(c, mx, my, mw / MIN(n, m->nmaster) - (2 * c->bw), mh - (2 * c->bw),
             0);
      if (mx + WIDTH(c) + m->gappx < m->mw)
        mx += WIDTH(c) + m->gappx;
    } else {
      resize(c, sx, sy, sw - (2 * c->bw),
             sh / (n - MIN(n, m->nmaster)) - (2 * c->bw), 0);
    ...
    if (i < m->nmaster) {
      ...
      if (m->nmaster >= n)
        resize(c, m->wx + mx + m->gappx, m->wy + my,
               mw - 2 * (c->bw + m->gappx), h - (2 * c->bw) - m->gappx, 0);
      else if (m->nmaster + 1 < n)
        resize(c, m->wx + mx + m->gappx / 2, m->wy + my,
               mw - 2 * c->bw - m->gappx, h - 2 * c->bw - m->gappx, 0);
      else
        resize(c, m->wx + mx + m->gappx, m->wy + my,
               mw - 2 * c->bw - m->gappx * 3 / 2, h - 2 * c->bw - m->gappx, 0);
      ...
      if ((i - m->nmaster) % 2) {
        ...
        resize(c, m->wx + m->gappx, m->wy + ety,
               tw - (2 * c->bw) - m->gappx * 3 / 2, h - 2 * c->bw - m->gappx,
               0);
        if (ety + HEIGHT(c) + m->gappx < m->mh)
          ety += HEIGHT(c) + m->gappx;
      } else {
        h = (m->wh - oty) / ((1 + n - i) / 2);
        resize(c, m->wx + mx + mw + m->gappx / 2, m->wy + oty,
               tw - (2 * c->bw) - m->gappx * 3 / 2, h - 2 * c->bw - m->gappx,
               0);
